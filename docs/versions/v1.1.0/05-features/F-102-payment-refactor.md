...
将自助洗衣业务的支付模块从硬编码的单网点单账户模式，重构为以微信支付服务商模式为核心、支付宝为补充的多模式收款体系。核心能力包括四部分：支付配置中心、微信服务商分账、微信服务商直收、支付宝直收。每个网点可独立绑定微信渠道配置一个（分账或直收二选一）和支付宝渠道配置一个（可选，最多两个渠道），改变以往改一个网点收款参数就要全量上线发布的困境。
...
> 作为 **用户**，我希望 **在洗衣网点既能用微信也能用支付宝支付**，以便 **不用因为支付方式不对而放弃洗衣**。
...
独立于微信支付体系，支持支付宝当面付和 APP 支付，作为网点的第二种收款渠道。网点绑定支付宝配置后，用户可在支付时选择支付宝。
...
2. 用户扫码后支付页仅展示「微信支付」选项，不展示支付宝
3. 若支付下单接口收到 `channel=alipay` 请求，返回错误码 400「该网点暂不支持支付宝支付」
...
### 整体
- [ ] F-102-AC16：网点微信+支付宝双渠道绑定后，用户可自选支付方式
